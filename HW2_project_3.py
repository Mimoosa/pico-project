...
class Pico:
    def __init__(self, sw1_pin, sensor_pin, rot_a, rot_b, e_switch):
        ...
        self.sensor_timer = None # Timer for screen updates
    ...
    def set_threshold(self):
        self.max = max(self.sensor_fifo.data)
        self.min = min(self.sensor_fifo.data)
        h = self.max - self.min # Calculate range
            ...
            if num_samples > 0: # Ensure the sample distance is positive
                ...
                if ppi_seconds > 0: # If the time between peaks is positive
                    ...
                    if hr > 60 and hr < 100: # Only consider valid heart rates
                        ...
                        if self.highlight == 1 or self.highlight == 2: # If the HRV analysis menu is selected
...
while True:
    ...
    if pico.encoder_fifo.has_data():
        ...
        if encoder_data == -1: # Check for counter-clockwise turn event
            ...
            if pico.highlight < 3:  # Ensure it doesn't exceed the menu options
